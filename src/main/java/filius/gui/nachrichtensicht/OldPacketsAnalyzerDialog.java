...
public class OldPacketsAnalyzerDialog extends JFrame implements AbstractPacketsAnalyzerDialog, I18n {
	
	private JTabbedPane tabbedPane;

	private static OldPacketsAnalyzerDialog exchangeDialog = null;

	private Hashtable<String, JPanel> openedTabs = new Hashtable<String, JPanel>();

	private Hashtable<String, OldPacketsAnalyzerTable> tabellen = new Hashtable<String, OldPacketsAnalyzerTable>();

	public static synchronized OldPacketsAnalyzerDialog getInstance(Frame owner) {
		if (exchangeDialog == null) {
			exchangeDialog = new OldPacketsAnalyzerDialog(owner);
		}
	...
	public void reset() {
		if (exchangeDialog != null) {
			exchangeDialog.setVisible(false);
		}

		exchangeDialog = null;
	}

	private OldPacketsAnalyzerDialog(Frame owner) {
		super();
	...
	public void addTable(SystemSoftware system, String identifier) {
		OldPacketsAnalyzerTable tabelle;
		JPanel panel;
		JScrollPane scrollPane;

		if (openedTabs.get(identifier) == null) {
			tabelle = new OldPacketsAnalyzerTable(this, identifier);
			tabelle.update();
