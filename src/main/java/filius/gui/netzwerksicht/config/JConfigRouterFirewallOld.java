...
public class JConfigRouterFirewallOld extends JDialog implements I18n {
	
	// Partially replaced by JConfigRouterFirewall
	// Still required by JTableEditable
...
//	    private String [] values = {"First", "Second", "Third"};
	...
	public JConfigRouterFirewallOld(Firewall firewall, JFrame dummyFrame) {
		...
		// Do close when Escape key is pressed
        ...
        // Force the width of the dialog window (but with glitches)
        addComponentListener(new ComponentAdapter() {
            public void componentResized(ComponentEvent e) {
            	setSize(new Dimension(720, getHeight())); 
            }
        });
	...
	private Box initFirewallRuleBox() {
		...
		defaultPolicyCombo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				firewall.setDefaultActionAsString((String) defaultPolicyCombo.getSelectedItem());
				updateRuleTable();
			}
		});
		...
		scrollPane.setPreferredSize(new Dimension(720, 2000));
	...
	private void initComponents() {
		...
		tp.add(messages.getString("jfirewalldialog_msg44"), initNICBox());
		//tp.setBackgroundAt(0, TAB_COLOR);
		
		tp.add(messages.getString("jfirewalldialog_msg21"), initFirewallRuleBox());
	...
	private void updateCellColors() {
		...
		for(int i=0; i<ruleTable.getRowCount(); i++) {
			if(EntryValidator.isValid((String) ruleTable.getValueAt(i,1), EntryValidator.musterIpAdresseAuchLeer))
				cellSrcIP.resetColor(i, 1);
			else
				cellSrcIP.setColor(i, 1, EntryValidator.wrongTextColor);
			if(! ((String) ruleTable.getValueAt(i,1)).isEmpty())
				if(((String) ruleTable.getValueAt(i,2)).isEmpty())
					cellSrcMask.setColor(i, 2, EntryValidator.wrongTextColor);
				else if(! EntryValidator.isValidSubnetmask((String) ruleTable.getValueAt(i, 2)))
					cellSrcMask.setColor(i, 2, EntryValidator.wrongTextColor);
				else
					cellSrcMask.resetColor(i, 2);
			else
				cellSrcMask.resetColor(i, 2);
			if(EntryValidator.isValid((String) ruleTable.getValueAt(i,3), EntryValidator.musterIpAdresseAuchLeer))
				cellDestIP.resetColor(i, 3);
			else
				cellDestIP.setColor(i, 3, EntryValidator.wrongTextColor);
			if(! ((String) ruleTable.getValueAt(i,3)).isEmpty())
				if(((String) ruleTable.getValueAt(i,4)).isEmpty())
					cellDestMask.setColor(i, 4, EntryValidator.wrongTextColor);
				else if(! EntryValidator.isValidSubnetmask((String) ruleTable.getValueAt(i, 4)))
					cellDestMask.setColor(i, 4, EntryValidator.wrongTextColor);
				else
					cellDestMask.resetColor(i, 4);
			else
				cellDestMask.resetColor(i, 4);
			if(EntryValidator.isValid(((String) ruleTable.getValueAt(i,6)).trim(), EntryValidator.musterPortAuchLeer))
				cellPort.resetColor(i, 6);
			else
				cellPort.setColor(i, 6, EntryValidator.wrongTextColor);
		}
	}
	...
	public void updateRuleTable() {
		...
		defaultPolicyCombo.setSelectedItem(firewall.getDefaultActionAsString());
