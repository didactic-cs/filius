...
public class Router extends InternetNode implements I18n {

    public static final String TYPE = messages.getString("hw_vermittlungsrechner_msg1");

    public Router() {
        super();
        Main.debug.println("INVOKED-2 (" + this.hashCode() + ") " + getClass() + " (Router), constr: Router()");

        this.createNICList(1);
        this.setSystemSoftware(new RouterOS());
    ...
    public List<String> getMACList() {	
    	...
    	int count = getNICList().size();
    	for (int i = 0; i<count; i++) macs.add(getNICList().get(0).getMac());
		return macs;
	}
