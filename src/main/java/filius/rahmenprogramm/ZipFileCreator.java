...
public class ZipFileCreator {
	 ...
	 * @param dir
	 *            Verzeichnis, dessen Inhalt in eine Zip-Datei eingelesen werden
	 *            soll.
	 * @param zipFileName
	 *            Der Name, den die Zip-Datei haben soll.
	 * @throws IOException
	 */
	public ZipFileCreator(String verzeichnis, String zipFileName) throws IOException {
		Main.debug.println("INVOKED (" + this.hashCode() + ") " + getClass() + ", constr: ErzeugeZipDatei("
		        + verzeichnis + "," + zipFileName + ")");
	...
	public void zipDatei(File verzeichnisZip, ZipOutputStream zipOut) throws IOException {
		Main.debug.println("INVOKED (" + this.hashCode() + ") " + getClass() + ", zipDatei(" + verzeichnisZip + ","
		        + zipOut + ")");
		
		byte[] buf = new byte[4096];
		// Alle Dateinamen aus dem Verzeichnis werden in des Array eingelesen.
		File[] fileArray = verzeichnisZip.listFiles();
		String fileName = "";
