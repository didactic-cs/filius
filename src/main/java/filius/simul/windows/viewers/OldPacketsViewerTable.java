...
public class OldPacketsAnalyzerTable extends JTable implements PacketAnalyzerObserver, I18n {
    ...
    public OldPacketsAnalyzerTable(JFrame dialog, String macAddress) {
        super();
        ...
        this.setDefaultRenderer(Object.class, new PacketsAnalyzerTableCellRenderer());
        initKontextMenue();
    ...
    private OldPacketsAnalyzerTable getTabelle() {
        return this;
    }
    ...
    public synchronized void update() {
        ...
        daten = PacketAnalyzer.getInstance().getDataEntries(interfaceId, false);
        ...
        for (int i = 0; i < daten.length; i++) {
            ...
            if (daten[i][SCHICHT_SPALTE].equals(PacketAnalyzer.PROTOKOLL_SCHICHTEN[0])) {
                if (netzzugangVisible) {
                    isValid = true;
                }
            } else if (daten[i][SCHICHT_SPALTE].equals(PacketAnalyzer.PROTOKOLL_SCHICHTEN[1])) {
                if (vermittlungVisible) {
                    isValid = true;
                }
            } else if (daten[i][SCHICHT_SPALTE].equals(PacketAnalyzer.PROTOKOLL_SCHICHTEN[2])) {
                if (transportVisible) {
                    isValid = true;
                }
            } else if (daten[i][SCHICHT_SPALTE].equals(PacketAnalyzer.PROTOKOLL_SCHICHTEN[3])) {
                if (anwendungVisible) {
                    isValid = true;
                }
            } else {
