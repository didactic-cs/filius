 ...
 * dafuer gesorgt, dass die dazu gehoerigen eingehenden Antworten verarbeitet werden. Dazu wird ein neuer Thread mit
...
class PeerToPeerClient extends Anwendung {
    ...
    /**
     * Eine Liste mit allen Mitarbeiter-Threads, die zur Verarbeitung der Antworten auf eigene Anfragen verwendet
     * werden.
     */
    private LinkedList<PeerToPeerClientMitarbeiter> mitarbeiterEigeneAnfragen = new LinkedList<PeerToPeerClientMitarbeiter>();

    /**
     * Eine Liste mit allen Mitarbeiter-Threads, die zur Verarbeitung von Antworten auf fremde, d. h. weitergeleitete,
    ...
    /**
     * Konstruktor, in dem die zugehoerige PeerToPeerAnwendung initialisiert und der Konstruktor der Oberklasse
     * aufgerufen wird.
     * 
    ...
    /**
     * Verschicken eines eigenden oder weitersenden einer fremden Ping-Nachricht. Wenn eine IP-Adresse angegeben wird,
     * zu dem diese Nachricht verschickt werden soll, wird die Ping-Nachricht als eigene Anfrage behandelt. Eine
     * Ping-Nachricht wird zum Beitreten zu einem Peer-To-Peer-Netzwerk verschickt.
     * 
     * @param wohinZuerst
     *            die IP-Adresse des Servents, mit welchem man sich zuerst verbinden moechte
     * @param pingPaket
     *            das Ping-Paket, welches verschickt wird
     * @param peerToPeerServerMitarbeiter
     *            der peerToPeerServerMitarbeiter, ueber den eine moegliche Antwort zurueck geschickt wird
     * @param nichtIP
    ...
    /**
     * Damit wird eine initiierte Suchanfrage abgebrochen. D. h., dass alle Threads, die auf eingehende Antworten auf
    ...
    /**
     * erstellt eine HTTP Nachricht und schickt sie an den Teilnehmer, auf welchem die entsprechende Datei liegt. Auch
     * dazu wird ein Mitarbeiter-Thread gestartet und dieser in die Liste der Threads fuer eigene Anfragen eingefuegt.
     * 
     * @param teilnehmerIp
    ...
    /**
     * Initiierung und weiterleitung einer Suchanfrage (Query). Zur weiteren Verarbeitung wird ein neuer
     * Mitarbeiter-Thread erzeugt. Der Thread wird der Liste fuer eigene Anfragen hinzugefuegt, wenn die GUID in der
     * Liste der eigenen Anfragen der PeerToPeerAnwendung vorhanden ist. Die Anfrage wird immer an alle bekannten
     * Teilnehmer im Peer-To-Peer-Netzwerk verschickt.
     * 
     * @param anfragePaket
