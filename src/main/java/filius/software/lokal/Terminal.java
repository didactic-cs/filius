...
public class Terminal extends ClientApplication implements I18n {

    // Betriebssystem betriebssystem;
    boolean abfrageVar;

    private FiliusFileNode currentDirectory;
	...
	public String mv(String[] args) {
        ...
        if (!numParams(args, 2)) {
            notifyObservers(messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg40"));
...
//                            + FiliusFileSystem.FILE_SEPARATOR + args[0]);        	
        	String path = getCurrentDirectory().toPath() + FFS.FILE_SEPARATOR + args[0];
            FFS.deleteFile(path);            
            
            notifyObservers(messages.getString("sw_terminal_msg35"));
            return messages.getString("sw_terminal_msg35");
        } else {
            notifyObservers(messages.getString("sw_terminal_msg36"));
    ...
    public String del(String[] args) {
        ...
        if (!numParams(args, 1)) {
            notifyObservers(messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg41"));
        ...
        if (FFS.deleteFile(getCurrentDirectory().toPath() + FFS.FILE_SEPARATOR + args[0])) {
            notifyObservers(messages.getString("sw_terminal_msg37"));
            return messages.getString("sw_terminal_msg37");
        } else {
            notifyObservers(messages.getString("sw_terminal_msg38"));
    ...
    private boolean pureCopy(String[] args) {
        ...
        if (srcString.length() > 0 && srcString.substring(0, 1).equals(FFS.FILE_SEPARATOR)) { // 'pfad'
            ...
            srcString = FFS.evaluatePath(getCurrentDirectory().toPath() + FFS.FILE_SEPARATOR + srcString);
        }
        ...
        if (destString.length() > 0 && destString.substring(0, 1).equals(FFS.FILE_SEPARATOR)) { // 'pfad'
            ...
            destString = FFS.evaluatePath(getCurrentDirectory().toPath() + FFS.FILE_SEPARATOR + destString);
        }
    ...
    public String cp(String[] args) {
        ...
        if (pureCopy(args)) {
            notifyObservers(messages.getString("sw_terminal_msg33"));
            return messages.getString("sw_terminal_msg33");
        } // positive case, everything worked fine
        else {
            notifyObservers(messages.getString("sw_terminal_msg34"));
    ...
    public String ipconfig(String[] args) {
        ...
        Main.debug.println(")");
        if (!numParams(args, 0)) {
            notifyObservers(messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg42"));
            return messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg42"); // wrong
                                                                                                      // number
                                                                                                      // of
                                                                                                      // parameters
        }
        HostOS bs = (HostOS) getSystemSoftware();
        String ausgabe = "";

        ausgabe += messages.getString("sw_terminal_msg4") + " " + bs.getIPAddress() + "\n";
        ausgabe += messages.getString("sw_terminal_msg5") + " " + bs.getSubnetMask() + "\n";
        ausgabe += messages.getString("sw_terminal_msg26") + " " + bs.getMACAddress() + "\n";
    ...
    public String route(String[] args) {
        ...
        LinkedList<String[]> routingTabelle = getSystemSoftware().getRoutingTable().getRouteList();
    ...
    public String dir(String[] args) {
        ...
        if (!numParams(args, 0, 1)) {
            notifyObservers(messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg43"));
        ...
        if (args[0].isEmpty()) {
            liste = currentDirectory.getChildObjects();        	
            currPath = currentDirectory.toPath();
        } else {
            if (FFS.isAbsolute(args[0])) {
            	...
            	// Relative path
            	FiliusFileNode node = currentDirectory.toNode(args[0]); 
            	if (node != null) liste = node.getChildObjects();
            	else liste = null;            
                currPath = FFS.evaluatePath(currentDirectory.toPath() + FFS.FILE_SEPARATOR + args[0]);
            }
        }

        if (liste == null || liste.size() == 0) {
            notifyObservers(messages.getString("sw_terminal_msg8"));
        ...
        notifyObservers(inhalt.toString());
    ...
    public String touch(String[] args) {
        ...
        if (args[0].length() > 0 && args[0].substring(0, 1).equals(FFS.FILE_SEPARATOR)) { // 'pfad'
            ...
            absPath = FFS.evaluatePath(currentDirectory.toPath() + FFS.FILE_SEPARATOR + args[0]);
        }
    ...
    public String cd(String[] args) {
        ...
        if (numParams(args, 1)) {
            ...
            if (FFS.isAbsolute(args[0])) 
                ...
                newDir = currentDirectory.toNode(args[0]);
            ...
            ergebnis = currentDirectory.toPath();
        }
    ...
    public String pwd(String[] args) {
        ...
        String ergebnis = currentDirectory.toPath();
        notifyObservers(ergebnis);
        return ergebnis;
    }
    ...
    public String netstat(String[] args) {
        ...
        transport = this.getSystemSoftware().getTcp();
        protocol = "TCP";
        processSockets(ergebnis, transport, protocol);

        transport = this.getSystemSoftware().getUdp();
        protocol = "UDP";
        processSockets(ergebnis, transport, protocol);
    ...
    private SocketInformation getSocketInfo(Socket socket) {
        ...
        try {
            Route routingEntry = ((InternetNodeOS) this.getSystemSoftware())
    ...
    public String help(String[] args) {
        notifyObservers(messages.getString("sw_terminal_msg25"));
    ...
    public String cat(String[] args) {
        ...
        if (args == null || args.length < 1 || args[0] == null || "".equals(args[0])) {
            ...
            FiliusFile file = currentDirectory.getFiliusFile(args[0]);
            if (null != file) {
    ...
    public String arp(String[] args) {
        ...
        Map<String, String> arpTable = this.getSystemSoftware().getARP().holeARPTabelle();
        ...
        ergebnis.append("----------------------------------------\n");
        notifyObservers(ergebnis.toString());
        return ergebnis.toString();
    }
    ...
    public String host(String[] args) {
        ...
        Main.debug.println(")");
        if (!numParams(args, 1)) {
            notifyObservers(messages.getString("sw_terminal_msg32") + messages.getString("sw_terminal_msg44"));
        ...
        HostOS bs = (HostOS) getSystemSoftware();
        filius.software.dns.Resolver res = bs.getDNSClient();
        if (res == null) {
            filius.Main.debug.println("ERROR (" + this.hashCode() + "): Terminal 'host': Resolver is null!");
            notifyObservers(messages.getString("sw_terminal_msg30"));
            return messages.getString("sw_terminal_msg30");
        }
        String result;

        try {
            result = res.getIPAdresse(args[0]);
            if (result != null) {
                notifyObservers(args[0] + " " + messages.getString("sw_terminal_msg28") + " " + result + "\n");
            ...
            notifyObservers(messages.getString("sw_terminal_msg31"));
            return messages.getString("sw_terminal_msg31");
        } catch (Exception e) {
            e.printStackTrace(filius.Main.debug);
            notifyObservers(messages.getString("sw_terminal_msg29"));
    ...
    public String ping(String[] args) {
        ...
        Resolver res = getSystemSoftware().getDNSClient();
        if (res == null) {
        ...
        try {
            ...
            if (destIp == null) { // args[0] is not an IP address
                destIp = res.getIPAdresse(args[0]);
            }
            ...
            notifyObservers(messages.getString("sw_terminal_msg31") + " (DNS)");
        ...
        try {
            Route route = ((InternetNodeOS) getSystemSoftware()).determineRoute(destIp);
            if (VermittlungsProtokoll.isBroadcast(destIp, route.getInterfaceIpAddress(), route.getNetMask())) {
                notifyObservers(messages.getString("sw_terminal_msg53"));
                return messages.getString("sw_terminal_msg53");
            }
        } catch (RouteNotFoundException e1) {
            notifyObservers(messages.getString("sw_terminal_msg52"));
        ...
        notifyObservers("PING " + args[0] + " (" + destIp + ")");
        ...
        for (num = 0; !interrupted && num < loopNumber; num++) {
            try {
                ...
                // Ergebnis da ist!!!
                int resTTL = getSystemSoftware().getICMP().startSinglePing(destIp, num + 1);
        ...
        notifyObservers(Boolean.FALSE); // inform about a multiple
    ...
    public String traceroute(String[] args) {
        if (!numParams(args, 1)) {
            notifyObservers(messages.getString("sw_terminal_trace_msg1"));
            return null;
        }
        ...
        if (destIP == null) {
            filius.software.dns.Resolver res = getSystemSoftware().getDNSClient();

            try {
                destIP = res.getIPAdresse(args[0]);
            } catch (TimeoutException e) {
                notifyObservers(messages.getString("sw_terminal_trace_msg2"));
                return null;
            }

        }
        if (destIP == null) {
            notifyObservers(messages.getString("sw_terminal_trace_msg3"));
            return null;
        }
        ...
        for (ttl = 1; ttl <= maxHops && !interrupted; ttl++) {
            ...
            for (int i = 0; i < 3 && !interrupted; i++) {
                seqNr++;
                recv = getSystemSoftware().getICMP().sendProbe(destIP, ttl, seqNr);
            ...
            if (fehler == 0) {
                notifyObservers(recv.getQuellIp());
    ...
    public FiliusFileNode getCurrentDirectory() {
        return currentDirectory;
    }

    public void setCurrentDirectory(FiliusFileNode currentDirectory) {
