 ...
 */
public class ModemReceiver extends ProtocolThread {
     ...
     * aufgerufen.
     */
    public ModemReceiver(ModemFirmware firmware, InputStream in) {
    ...
    protected void processFrame(Object datenEinheit) {
        ...
        if (firmware.isStarted()) {
            ...
            synchronized (((Modem) firmware.getNode()).getPort().getOutputBuffer()) {
                ((Modem) firmware.getNode()).getPort().getOutputBuffer().add(frame);
                ((Modem) firmware.getNode()).getPort().getOutputBuffer().notify();
            }
        }
    }
